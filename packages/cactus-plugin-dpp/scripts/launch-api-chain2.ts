...
async function syncMetadataToChain1(
  ...
  console.log(`[metadata-sync] Session done. Syncing metadata for DPP ${dppId} to chain 1…`);

  const newData = { productName: snap.productName, ...(snap.publicData || {}) };

  try {
    const res = await fetch(`${CHAIN1_API}${DPP_API_BASE}/amend`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ dppId, newData }),
    });
    if (res.ok) {
      console.log(`[metadata-sync] Metadata amended on chain 1 for DPP ${dppId}`);
    } else {
  ...
  for (const cert of snap.certifications) {
    try {
      await fetch(`${CHAIN1_API}${DPP_API_BASE}/add-certification`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
async function main() {
  ...
  app.post(`${base}/cross-chain-transfer`, async (req, res) => {
    try {
      ...
      // Snapshot metadata BEFORE the lock so we can restore it on chain 1
